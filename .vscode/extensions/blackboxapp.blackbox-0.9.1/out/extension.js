					...
					async (data) => {
						...
						if (data.command) {
							...
							if (data.command =='search') open('https://www.useblackbox.io/codesearchdemo')
								...
								open(`https://www.useblackbox.io/signup?ref=tryvscode&vsId=${userId}`)
								upgradeFct(userId)
							}
			...
			<body>
				...
				<div>
					...
					<div class="btn-default btn-features">
						<span class="btn-features">Code Autcomplete</span>
					</div>
				</div>
				<script>
					(function() {
						...
						document.addEventListener("click", (e)=>{
							if(e.target.classList.contains("btn-features") ) postMessage({command: "features"})
